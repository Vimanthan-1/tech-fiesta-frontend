...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  // Maintain latest callback refs so we don't recreate the WebGL context when they change
  ...
  // Main WebGL Initialization and Lifecycle Effect
  useEffect(() => {
    ...
    try {
      ...
      const animate = () => {
        ...
        const timeVal = timeOffset + clock.getElapsedTime() * speedRef.current;
        uniforms.uTime.value = timeVal;
        if (liquidEffect) {
          const timeUniform = liquidEffect.uniforms.get('uTime');
          if (timeUniform) timeUniform.value = timeVal;
        }
        if (composer) {
          if (touch) touch.update();
          composer.passes.forEach(p => {
            const effs = (p as unknown as { effects?: unknown[] }).effects;
            if (Array.isArray(effs)) {
              effs.forEach((eff) => {
                const u = (eff as unknown as { uniforms?: Map<string, THREE.IUniform<number>> }).uniforms?.get('uTime');
                if (u) u.value = timeVal;
              });
            }
          });
          composer.render();
      ...
      console.warn("PixelBlast WebGL initialization failed (falling back to static background):", err);
      if (onReadyRef.current) {
        onReadyRef.current();
      }
    }
  ...
  // Separate effect to handle uniform updates dynamically without re-initializing WebGL
  useEffect(() => {
    const t = threeRef.current;
    if (!t) return;
